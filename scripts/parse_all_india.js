...
async function parseAllIndia() {
  const pdfjsLib = await import('pdfjs-dist/legacy/build/pdf.mjs');
  const filePath = path.join(__dirname, 'docs', 'all_india_provisional_result.pdf');
  ...
  for (let p = 3; p <= doc.numPages; p++) {
    ...
    if (p % 100 === 0 || p === doc.numPages) {
      console.log(`Processed page ${p}/${doc.numPages}... Total records parsed so far: ${records.length}`);
    }
  }
  ...
  console.log(`\nParsing complete! Total records: ${records.length}`);
  ...
  // Save to JSON
  fs.writeFileSync(path.join(__dirname, 'docs', 'all_india_provisional_result.json'), JSON.stringify(records, null, 2), 'utf8');
  ...
  const csvPath = path.join(__dirname, 'docs', 'all_india_provisional_result.csv');
  fs.writeFileSync(csvPath, csvLines.join('\n'), 'utf8');
  console.log(`Saved CSV to: ${csvPath}`);
}

parseAllIndia().catch(console.error);
