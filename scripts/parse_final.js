...
async function parseAllIndiaFinal() {
  const pdfjsLib = await import('pdfjs-dist/legacy/build/pdf.mjs');
  const filePath = path.join(__dirname, 'docs', 'all_india_provisional_result_final.pdf');
  const data = new Uint8Array(fs.readFileSync(filePath));
  const doc = await pdfjsLib.getDocument({ data }).promise;

  console.log(`Loaded Final Result PDF with ${doc.numPages} pages.`);
  ...
  for (let p = 3; p <= doc.numPages; p++) {
    ...
    for (let row of rows) {
      ...
      if (
        ...
        rowStr.includes('NEET-UG Counselling Seats Allotment') ||
  ...
  console.log(`\nParsing complete! Total records in Final Result: ${records.length}`);
  ...
  // Save to JSON
  fs.writeFileSync(path.join(__dirname, 'docs', 'all_india_provisional_result_final.json'), JSON.stringify(records, null, 2), 'utf8');
  ...
  const csvPath = path.join(__dirname, 'docs', 'all_india_provisional_result_final.csv');
  fs.writeFileSync(csvPath, csvLines.join('\n'), 'utf8');
  console.log(`Saved Final CSV to: ${csvPath}`);
}

parseAllIndiaFinal().catch(console.error);
